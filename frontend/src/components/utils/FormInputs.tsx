    ...
    <Controller
      render={(props) => (
        <Autocomplete
          ...
          //     renderOption={(option) => ()} could be useful later
          renderInput={(params) => (
            <TextField
              {...params}
              fullWidth={true}
...
export function IdInput(props: InputProps) {
  return (
    <Controller
      name={props.name}
      as={<input id={props.name} type="number" hidden />}
...
export function SelectInput(selectProps: SelectProps) {
  ...
  return (
    <Controller
      ...
      as={
        <Select id={selectProps.name} className={selectProps.className}>
          {selectProps.selectOptions.map((option, index) => (
