...
export function createParentQueryString<T>(params: QueryStringParameters<T>) {
    ...
    const sorterString = (sorting && sorting.sortParam && sorting.order) ? `&sortParam=${sorting.sortParam}&sortOrder=${sorting.order}` : ''
    const filterString = (filtering && filtering.paramName && filtering.paramValue) ? `&${filtering.paramName}=${filtering.paramValue}` : ''
