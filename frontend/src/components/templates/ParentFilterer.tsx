            ...
            <Grid item>
                <Select
                    ...
                    onChange={(event: React.ChangeEvent<{ value: unknown }>) => {
                        ...
                        if (newValue === '' || newValue === 'beforeDate' || newValue === 'afterDate' || newValue === 'date') {
    ...
    return (
        <SimpleDialog title={props.title}
                      renderOpenButton={(dialogOpen, setDialogOpen, className) =>
                          (<Button onClick={setDialogOpen} hidden={dialogOpen} className={className}>{props.openButtonLabel}</Button>)}
                      afterSubmit={data => {
                      }}
                      render={(afterSubmitFn, closeDialogFn) => (
                          <>
                              <ParentFilterer filterValue={filterValue} setFilterValue={setFilterValue}
