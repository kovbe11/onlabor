        ...
        <Grid container spacing={1}>
            <Grid item>
                ...
                >
                    ...
                    <MenuItem key="No order" value="">No sorting</MenuItem>
                </Select>
            </Grid>
            <Grid item>
                <Select
                ...
                >
                    <MenuItem key="No sorting" value="">No sorting</MenuItem>
                    {
                        props.sortingParamOptions.map(
                            (option: Option) => <MenuItem key={option.value}
...
export function ParentSorterDialog(props: ParentSorterDialogProps) {
    ...
    return (
        <SimpleDialog title={props.title}
                      renderOpenButton={(dialogOpen, setDialogOpen, className) =>
                          (<Button onClick={setDialogOpen} hidden={dialogOpen}
                                   className={className}>{props.openButtonLabel}</Button>)}
                      afterSubmit={data => {
                      }}
                      render={(afterSubmitFn, closeDialogFn) => (
                          <>
                              <ParentSorter sortingParam={sortingParam} setSortingParam={setSortingParam}
                                            order={sortingOrder} setOrder={setSortingOrder}
