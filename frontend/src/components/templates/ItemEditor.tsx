...
export const onSubmitEdit = (navigate: () => void, setLoading: (loading: boolean) => void, props: SubmitEditProps) => {
    let {mapper, apiPrefix, id} = props

    return ((data: any) => {
        setLoading(true)
        let mappedData = mapper(data)
        api.put(apiPrefix + '/' + id, mappedData)
            .then(r => {
                console.log(r)
                navigate()
            })
    ...
    return (
        <Paper>
            {loading ? (<CircularProgress/>) :
                (
                    <ItemEditor renderInputs={props.renderInputs}
                                errors={errors}
